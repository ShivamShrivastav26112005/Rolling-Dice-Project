...
document.addEventListener("DOMContentLoaded", function () { // This ensures the script runs only after the entire HTML page is loaded.
  
  // 2. Get HTML Elements by ID
  ...
  // 3 functions -: 
  // rollDice() → Rolls the dice when "Roll" is clicked.
  // playAgain() → Starts a new game when "Play Again" is clicked.
  // playNextRound() → Moves to the next round when "Next Round" is clicked.



  // 5. Function to Roll the Dice. 
  ...
  function rollDice() {
    if (!winner) {
      ...
      if (currentPlayer === 1) {
        player1.style.backgroundColor = "#7c5ea475";
        player2.style.backgroundColor = "#581845";
      } else {
        player1.style.backgroundColor = "#581845";
        player2.style.backgroundColor = "#7c5ea475";
      }
  ...
  function resetPoints() {
    ...
    for (let i = 0; i < 3; i++) {
      ...
      playerOnePoints.children[i].style.backgroundColor = "#85256b";
      playerTwoPoints.children[i].style.backgroundColor = "#85256b";
    }
  }
